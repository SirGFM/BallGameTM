...
public class Options : VerticalTextMenu {
	...
	override protected void start() {
		...
		Option[] _opts = {
			...
			new Option("MouseCam",
					"Whether mouse camera controls are always active/only on press. Try it out!",
					(new Values(idx => Config.setContinuousMouseCamera(idx == 1),
								"On Press",
								"Always On")).setAt((Config.getContinuousMouseCamera()) ? 1 : 0)),
			new Option("Timer",
